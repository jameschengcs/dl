...
model = Sequential()  
model.add(Conv2D(16, 3, 3, input_shape = (imgW, imgH, imgCh), kernel_initializer = initializer, activation = 'relu'))
...
model.add(Conv2D(8, 3, 3, kernel_initializer = initializer, activation = 'relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
...
opt = tf.keras.optimizers.Adam(learning_rate=0.01, beta_1=0.01, beta_2=0.29, amsgrad=True)
...
model.fit(X, Y, epochs=30, batch_size=20,  shuffle = True, verbose=1)
